...
@section('title', __('Mensajes').' | CyberTechna Solutions')
            ...
            <article class="admin-card">
                <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 mb-3">
                    ...
                    <div class="text-lg-end">
                        <div class="badge text-bg-{{ $message->reviewed_at ? 'success' : 'warning' }} mb-2">{{ $message->reviewed_at ? __('Revisado') : __('Pendiente') }}</div>
                        <div class="muted small">{{ $message->created_at->format('d/m/Y H:i') }}</div>
                    </div>
                </div>

                <div class="mb-3">
                    <span class="soft-label">{{ __('Servicio') }}</span>
                ...
                @if (! $message->reviewed_at)
                    <form method="POST" action="{{ route('admin.messages.review', $message) }}">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-admin">{{ __('Marcar como revisado') }}</button>
                    </form>
                @endif
            </article>
        @empty
